...
: bandwidth_(8e9f), fps_(30.0f), lambda_(5.00)
{}
...
void DianePublisher::declareParameters(const std::string & base_topic)
{
  ...
  d.description = "Lambda params for diane";
  declareParam("lambda", lambda_, d);
  getParam   ("lambda", lambda_);
  ...
  for (const auto& p : v) {
             ...
             name.compare(name.size() - 3, 3, "fps") == 0)
    {
      ...
      RCLCPP_INFO(log, "FPS → %.0f", fps_);
       ...
       else if (name.size() > 3 && name.size()<=6 &&
             name.compare(name.size() - 6, 6, "lambda") == 0)
    {
      fps_ = static_cast<float>(p.as_double());
      RCLCPP_INFO(log, "lambda → %.0f", lambda_);
    }
  }
