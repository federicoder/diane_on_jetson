...
int64_t diane_multinomial_i16(
   ...
   // torch::Tensor& c,               // prealloc [N]
    ...
    float lambda,		     // lambda paramter to enstablish how aggressive is the algo
...
auto invdist = 1.0f / (z.div(max_z + 1e-6f).pow(lambda).add(1e-6f));
...
std::vector<uint8_t> encode_diane_multinomial_i16(
    ...
    int64_t bufSize = diane_multinomial_i16(
        pc, out, bw, fps, lambda, dist_guarant, dflt, nanf, N, 10.0f
    );
